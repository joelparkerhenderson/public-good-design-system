...
import Subject from "../../components/KanbanTable.vue";


describe("KanbanTable", () => {
    test("renders a grid", () => {
        render(Subject, { props: { label: "Board" }, slots: { default: "cols" } });
    ...
    test("has aria-label", () => {
        render(Subject, { props: { label: "Sprint 5 board" }, slots: { default: "cols" } });
        expect(screen.getByRole("grid").getAttribute("aria-label")).toBe("Sprint 5 board");
    });

    test("renders caption when provided", () => {
        render(Subject, { props: { label: "Board", caption: "Sprint 5 board" }, slots: { default: "cols" } });
    ...
    test("renders children content", () => {
        render(Subject, { props: { label: "Board" }, slots: { default: "Tasks here" } });
        expect(screen.getByText("Tasks here")).toBeTruthy();
    });

    test("passes through attributes", () => {
        render(Subject, { props: { label: "K", "data-testid": "kb" }, slots: { default: "x" } });
