...
import Subject from "../../components/Tour.vue";


describe("Tour", () => {
  test("renders dialog when active", () => {
    render(Subject, { props: { label: "Getting started",
        active: true }, slots: { default: "Tour content" } });
    expect(screen.getByRole("dialog")).toBeTruthy();
  });

  test("does not render when not active", () => {
    render(Subject, { props: { label: "Getting started",
        active: false }, slots: { default: "Tour content" } });
    expect(screen.queryByRole("dialog")).toBeNull();
  });

  test("defaults to not active", () => {
    render(Subject, { props: { label: "Tour" }, slots: { default: "content" } });
    expect(screen.queryByRole("dialog")).toBeNull();
  });
