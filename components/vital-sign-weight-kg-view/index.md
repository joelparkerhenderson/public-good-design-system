# Vital Sign Weight Kg View

VitalSignWeightKgView is a headless read-only display of a weight value in kilograms (kg). It renders a `<span>` with `role="img"` and an accessible label describing the value and unit.
...
- Companion to VitalSignWeightKgInput for the Input/View pattern
...
- Use as the display companion to VitalSignWeightKgInput.
- Avoid using for editable weight values; use VitalSignWeightKgInput instead.
...
The consumer provides all CSS styling. The component renders with a `.vital-sign-weight-kg-view` class for targeting. Common styling includes displaying "kg" as a unit label alongside the number.
