# Vital Sign Respiratory Rate Breaths Per Minute View

VitalSignRespiratoryRateBreathsPerMinuteView is a headless read-only display of a respiratory rate value in breaths per minute (bpm). It renders a `<span>` with `role="img"` and an accessible label describing the value and unit.
...
- Use as the display companion to VitalSignRespiratoryRateBreathsPerMinuteInput.
- Avoid using for editable respiratory rate values; use VitalSignRespiratoryRateBreathsPerMinuteInput instead.
...
The consumer provides all CSS styling. The component renders with a `.vital-sign-respiratory-rate-breaths-per-minute-view` class for targeting. Common styling includes color coding for ranges (green for normal 12-20 bpm, red for bradypnea <12 or tachypnea >20).
