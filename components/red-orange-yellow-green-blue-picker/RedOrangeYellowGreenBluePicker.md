# RedOrangeYellowGreenBlueStatusPicker
...
```svelte
<script lang="ts">
  import RedOrangeYellowGreenBlueStatusPicker from './RedOrangeYellowGreenBlueStatusPicker.svelte';

  let level = $state("");
</script>

<RedOrangeYellowGreenBlueStatusPicker label="Risk level" bind:value={level} />
...
```svelte
<RedOrangeYellowGreenBlueStatusPicker label="Alert status" value="green" />
```
