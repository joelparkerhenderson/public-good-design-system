# Vital Sign Height Cm View

VitalSignHeightCmView is a headless read-only display of a height value in centimetres (cm). It renders a `<span>` with `role="img"` and an accessible label describing the value and unit.
...
- Companion to VitalSignHeightCmInput for the Input/View pattern
...
- Use as the display companion to VitalSignHeightCmInput.
- Avoid using for editable height values; use VitalSignHeightCmInput instead.
...
The consumer provides all CSS styling. The component renders with a `.vital-sign-height-cm-view` class for targeting. Common styling includes displaying "cm" as a unit label alongside the number.
