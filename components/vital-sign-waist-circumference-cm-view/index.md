# Vital Sign Waist Circumference Cm View

VitalSignWaistCircumferenceCmView is a headless read-only display of a waist circumference value in centimetres (cm). It renders a `<span>` with `role="img"` and an accessible label describing the value and unit.
...
- Use as the display companion to VitalSignWaistCircumferenceCmInput.
- Avoid using for editable waist circumference values; use VitalSignWaistCircumferenceCmInput instead.
...
The consumer provides all CSS styling. The component renders with a `.vital-sign-waist-circumference-cm-view` class for targeting. Common styling includes color coding for ranges based on sex-specific thresholds (e.g., green for healthy, amber for high risk, red for very high risk).
