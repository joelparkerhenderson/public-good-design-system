# Vital Sign Cholesterol HDL Mmol Per Litre View

VitalSignCholesterolHdlMmolPerLitreView is a headless read-only display of an HDL cholesterol value in millimoles per litre (mmol/L). It renders a `<span>` with `role="img"` and an accessible label describing the value and unit.
...
- Companion to VitalSignCholesterolHdlMmolPerLitreInput for the Input/View pattern
...
- Use to display an HDL cholesterol reading in vital sign summaries, medical records, or health dashboards.
- Use as the display companion to VitalSignCholesterolHdlMmolPerLitreInput.
- Avoid using for editable cholesterol values; use VitalSignCholesterolHdlMmolPerLitreInput instead.
...
The consumer provides all CSS styling. The component renders with a `.vital-sign-cholesterol-hdl-mmol-per-litre-view` class for targeting. Common styling includes color coding for ranges (green for healthy >1.0 mmol/L, red for low <1.0 mmol/L).
