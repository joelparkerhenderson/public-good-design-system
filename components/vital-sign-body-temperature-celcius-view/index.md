# Vital Sign Body Temperature Celcius View

VitalSignBodyTemperatureCelciusView is a headless read-only display of a body temperature value in degrees Celsius. It renders a `<span>` with `role="img"` and an accessible label describing the value and unit.
...
- Companion to VitalSignBodyTemperatureCelciusInput for the Input/View pattern
...
- Use as the display companion to VitalSignBodyTemperatureCelciusInput.
- Avoid using for editable temperature values; use VitalSignBodyTemperatureCelciusInput instead.
...
The consumer provides all CSS styling. The component renders with a `.vital-sign-body-temperature-celcius-view` class for targeting. Common styling includes color coding for ranges (green for normal 36.1-37.2°C, red for fever ≥38.0°C or hypothermia ≤35.0°C).
