...
```svelte
<script lang="ts">
  import RedAmberGreenStatusPicker from './RedAmberGreenStatusPicker.svelte';

  let status = $state("");
</script>

<RedAmberGreenStatusPicker label="Project status" bind:value={status} />
...
```svelte
<RedAmberGreenStatusPicker label="Health check" value="green" />
```
