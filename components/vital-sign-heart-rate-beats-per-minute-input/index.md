# Vital Sign Heart Rate Beats Per Minute Input

VitalSignHeartRateBeatsPerMinuteInput is a headless number input for entering a heart rate value in beats per minute (BPM). It wraps a native `<input type="number">` with integer-only constraints and accessible labelling.
...
- Companion to VitalSignHeartRateBeatsPerMinuteView for the Input/View pattern
...
```html
<VitalSignHeartRateBeatsPerMinuteInput label="Heart rate (BPM)" value={value} />
```

```html
<VitalSignHeartRateBeatsPerMinuteInput label="Resting heart rate" value={rate} required />
```
...
- Use as the input companion to VitalSignHeartRateBeatsPerMinuteView.
...
The consumer provides all CSS styling. The component renders with a `.vital-sign-heart-rate-beats-per-minute-input` class for targeting. Common styling includes displaying "BPM" as a suffix label and validation indicators for out-of-range values.
