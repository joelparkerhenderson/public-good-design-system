# Vital Sign Cholesterol HDL Mmol Per Litre Input

VitalSignCholesterolHdlMmolPerLitreInput is a headless number input for entering an HDL cholesterol value in millimoles per litre (mmol/L). It wraps a native `<input type="number">` with decimal-precision constraints and accessible labelling.
...
```html
<VitalSignCholesterolHdlMmolPerLitreInput label="HDL cholesterol (mmol/L)" value={value} />
```
...
- Use as the input companion to VitalSignCholesterolHdlMmolPerLitreView.
...
The consumer provides all CSS styling. The component renders with a `.vital-sign-cholesterol-hdl-mmol-per-litre-input` class for targeting. Common styling includes displaying "mmol/L" as a suffix label.
