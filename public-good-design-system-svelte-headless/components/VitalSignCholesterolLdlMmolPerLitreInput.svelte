<script lang="ts">
    // VitalSignCholesterolLdlMmolPerLitreInput component
    //
    // A headless numeric input for entering an LDL cholesterol value in millimoles
    // per litre. Wraps a native <input type="number"> element with accessible
    // labelling and sensible defaults for LDL cholesterol entry (min 0, max 10,
    ...
    //   value — number | undefined, default undefined. Current LDL cholesterol value; bindable.
    ...
    //
    // Syntax:
    //   <VitalSignCholesterolLdlMmolPerLitreInput label="LDL cholesterol (mmol/L)" bind:value />
    //
    // Examples:
    //   <!-- Basic LDL cholesterol input -->
    //   <VitalSignCholesterolLdlMmolPerLitreInput label="LDL cholesterol (mmol/L)" bind:value />
    //
    //   <!-- Required LDL cholesterol input with custom range -->
    //   <VitalSignCholesterolLdlMmolPerLitreInput label="LDL cholesterol (mmol/L)" bind:value min={0.5} max={8} required />
    //
    // Keyboard:
...
<!-- VitalSignCholesterolLdlMmolPerLitreInput.svelte -->
<input
    class={`vital-sign-cholesterol-ldl-mmol-per-litre-input ${className}`}
    type="number"
    aria-label={label}
    bind:value
    {min}
