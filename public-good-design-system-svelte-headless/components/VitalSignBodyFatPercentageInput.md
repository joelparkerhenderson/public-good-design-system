...
VitalSignBodyFatPercentageInput is a headless component that wraps a native `<input type="number">` element for entering a body fat percentage (BFP) value. It provides accessible labelling and sensible defaults (min 0, max 100, step 0.1) with a bindable numeric value for two-way data flow.

This component is useful for patient intake forms, health assessments, fitness applications, clinical records, medical applications, or any form field that accepts a body fat percentage. It is the input companion to VitalSignBodyFatPercentageView.
...
```svelte
<VitalSignBodyFatPercentageInput label="Body fat percentage" bind:value />
```
