# VitalSignHeightCmInput

VitalSignHeightCmInput is a headless component that wraps a native `<input type="number">` element for entering a height value in centimeters. It provides accessible labelling and sensible defaults (min 0, max 300, step 1) with a bindable numeric value for two-way data flow.

This component is useful for patient intake forms, health assessments, fitness trackers, medical records, or any form field that accepts a height in centimeters. It is the input companion to VitalSignHeightCmView.
...
- Defaults: `min=0`, `max=300`, `step=1` for typical height ranges
...
- `value`: number | undefined (default: `undefined`) -- current height value; bindable with `bind:value`
...
```svelte
<VitalSignHeightCmInput label="Height (cm)" bind:value />
```
