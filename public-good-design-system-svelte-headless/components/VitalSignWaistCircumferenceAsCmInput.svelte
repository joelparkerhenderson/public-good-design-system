<script lang="ts">
    ...
    //   <VitalSignWaistCircumferenceAsCmInput label="Waist circumference (cm)" bind:value min={30} max={200} required />
    //
    // Keyboard:
...
<!-- VitalSignWaistCircumferenceAsCmInput.svelte -->
<input
    class={`vital-sign-waist-circumference-as-cm-input ${className}`}
    type="number"
    aria-label={label}
    bind:value
    {min}
