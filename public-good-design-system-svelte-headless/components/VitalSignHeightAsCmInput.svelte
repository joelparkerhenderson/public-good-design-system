<script lang="ts">
    // VitalSignHeightAsCmInput component
    //
    // A headless numeric input for entering a height value in centimeters.
    // Wraps a native <input type="number"> element with accessible labelling and
    // sensible defaults for height entry (min 0, max 300, step 1). It provides
    ...
    //   value — number | undefined, default undefined. Current height value; bindable.
    ...
    //   <VitalSignHeightAsCmInput label="Height (cm)" bind:value />
    //
    //   <!-- Required height input with custom range -->
    //   <VitalSignHeightAsCmInput label="Height (cm)" bind:value min={50} max={250} required />
    //
    // Keyboard:
...
<!-- VitalSignHeightAsCmInput.svelte -->
<input
    class={`vital-sign-height-as-cm-input ${className}`}
    type="number"
    aria-label={label}
    bind:value
    {min}
