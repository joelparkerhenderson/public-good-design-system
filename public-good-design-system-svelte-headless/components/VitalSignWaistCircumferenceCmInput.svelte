<script lang="ts">
    // VitalSignWaistCircumferenceCmInput component
    //
    // A headless numeric input for entering a waist circumference value in centimeters.
    // Wraps a native <input type="number"> element with accessible labelling and
    // sensible defaults for waist circumference entry (min 0, max 300, step 1). It provides
    ...
    //   value — number | undefined, default undefined. Current waist circumference value; bindable.
    ...
    //
    // Syntax:
    //   <VitalSignWaistCircumferenceCmInput label="Waist circumference (cm)" bind:value />
    //
    // Examples:
    //   <!-- Basic waist circumference input -->
    //   <VitalSignWaistCircumferenceCmInput label="Waist circumference (cm)" bind:value />
    //
    //   <!-- Required waist circumference input with custom range -->
    //   <VitalSignWaistCircumferenceCmInput label="Waist circumference (cm)" bind:value min={30} max={200} required />
    //
    // Keyboard:
...
<!-- VitalSignWaistCircumferenceCmInput.svelte -->
<input
    class={`vital-sign-waist-circumference-cm-input ${className}`}
    type="number"
    aria-label={label}
    bind:value
    {min}
