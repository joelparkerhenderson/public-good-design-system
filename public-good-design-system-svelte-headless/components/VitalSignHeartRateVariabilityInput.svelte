<script lang="ts">
    // VitalSignHeartRateVariabilityInput component
    //
    // A headless numeric input for entering a heart rate variability (HRV) value in
    // milliseconds. Wraps a native <input type="number"> element with accessible
    // labelling and sensible defaults for HRV entry (min 0, max 500, step 1). It
    // provides a bindable numeric value for two-way data flow. Commonly used in
    // patient intake forms, health assessments, fitness trackers, and medical records.
    //
    // Props:
    //   className — string, optional. CSS class name.
    //   label — string, required. Accessible name for the input via aria-label.
    //   value — number | undefined, default undefined. Current HRV value; bindable.
    ...
    //
    // Syntax:
    //   <VitalSignHeartRateVariabilityInput label="HRV (ms)" bind:value />
    //
    // Examples:
    //   <!-- Basic HRV input -->
    //   <VitalSignHeartRateVariabilityInput label="HRV (ms)" bind:value />
    //
    //   <!-- Required HRV input with custom range -->
    //   <VitalSignHeartRateVariabilityInput label="HRV (ms)" bind:value min={10} max={400} required />
    //
    // Keyboard:
...
<!-- VitalSignHeartRateVariabilityInput.svelte -->
<input
    class={`vital-sign-heart-rate-variability-input ${className}`}
    type="number"
    aria-label={label}
    bind:value
    {min}
