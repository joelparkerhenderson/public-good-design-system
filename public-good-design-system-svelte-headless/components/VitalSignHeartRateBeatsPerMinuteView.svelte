<script lang="ts">
    ...
    //
    // A read-only display of a heart rate value in beats per minute. This is a
    ...
    //   value — number, required. Heart rate value in beats per minute to display.
    //   label — string, required. Accessible description via aria-label (e.g., "Heart rate: 72 beats per minute").
    ...
    //
    // Examples:
    //   <!-- Display a normal resting heart rate -->
    //   <VitalSignHeartRateBeatsPerMinuteView value={72} label="Heart rate: 72 beats per minute" />
    //
    //   <!-- Display an elevated heart rate -->
    //   <VitalSignHeartRateBeatsPerMinuteView value={110} label="Heart rate: 110 beats per minute" />
    //
    // Keyboard:
...
<!-- VitalSignHeartRateBeatsPerMinuteView.svelte -->
<span
    class={`vital-sign-heart-rate-beats-per-minute-view ${className}`}
    role="img"
    aria-label={label}
    data-value={value}
    {...restProps}
