...
VitalSignHeartRateAsBeatsPerMinuteInput is a headless component that wraps a native `<input type="number">` element for entering a heart rate value in beats per minute. It provides accessible labelling and sensible defaults (min 0, max 300, step 1) with a bindable numeric value for two-way data flow.

This component is useful for patient intake forms, health assessments, fitness trackers, medical records, or any form field that accepts a heart rate in beats per minute. It is the input companion to VitalSignHeartRateAsBeatsPerMinuteView.
...
- Defaults: `min=0`, `max=300`, `step=1` for typical heart rate ranges
...
- `value`: number | undefined (default: `undefined`) -- current heart rate value; bindable with `bind:value`
