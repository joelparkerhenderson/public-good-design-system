<script lang="ts">
    ...
    //
    // A headless numeric input for entering a respiratory rate value in breaths per minute.
    // Wraps a native <input type="number"> element with accessible labelling and
    // sensible defaults for respiratory rate entry (min 0, max 100, step 1). It provides
    // a bindable numeric value for two-way data flow. Commonly used in patient
    // intake forms, health assessments, fitness trackers, and medical records.
    //
    // Props:
    ...
    //
    // Syntax:
    //   <VitalSignRespiratoryRateAsBreathsPerMinuteInput label="Respiratory rate (breaths/min)" bind:value />
    //
    // Examples:
    //   <!-- Basic respiratory rate input -->
    //   <VitalSignRespiratoryRateAsBreathsPerMinuteInput label="Respiratory rate (breaths/min)" bind:value />
    //
    //   <!-- Required respiratory rate input with custom range -->
    //   <VitalSignRespiratoryRateAsBreathsPerMinuteInput label="Respiratory rate (breaths/min)" bind:value min={4} max={60} required />
    //
    // Keyboard:
...
<!-- VitalSignRespiratoryRateAsBreathsPerMinuteInput.svelte -->
<input
    class={`vital-sign-respiratory-rate-as-breaths-per-minute-input ${className}`}
    type="number"
    aria-label={label}
    bind:value
    {min}
