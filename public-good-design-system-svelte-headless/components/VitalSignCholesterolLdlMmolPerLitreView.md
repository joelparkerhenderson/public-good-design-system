...
VitalSignCholesterolLdlMmolPerLitreView is a headless component that displays a read-only LDL cholesterol value in millimoles per litre. It renders a `<span>` element with `role="img"` and an accessible label for screen readers.

This component is useful for patient monitors, health dashboards, medical records, clinical applications, or any context that displays an LDL cholesterol reading. It is the display-only companion to VitalSignCholesterolLdlMmolPerLitreInput.
