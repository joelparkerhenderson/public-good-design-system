<script lang="ts">
    ...
    // with accessible labelling and sensible defaults for systolic blood pressure
    // entry (min 0, max 300, step 1). It provides a bindable numeric value for
    ...
    //   <VitalSignBloodPressureSystolicInput label="Systolic blood pressure (mmHg)" bind:value />
    //
    //   <!-- Required systolic blood pressure input with custom range -->
    //   <VitalSignBloodPressureSystolicInput label="Systolic blood pressure (mmHg)" bind:value min={60} max={250} required />
    //
    // Keyboard:
...
<!-- VitalSignBloodPressureSystolicInput.svelte -->
<input
    class={`vital-sign-blood-pressure-systolic-input ${className}`}
    type="number"
    aria-label={label}
    bind:value
    {min}
