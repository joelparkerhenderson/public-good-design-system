...
VitalSignWeightAsKgInput is a headless component that wraps a native `<input type="number">` element for entering a weight value in kilograms. It provides accessible labelling and sensible defaults (min 0, max 500, step 1) with a bindable numeric value for two-way data flow.

This component is useful for patient intake forms, health assessments, fitness trackers, medical records, or any form field that accepts a weight in kilograms. It is the input companion to VitalSignWeightAsKgView.
...
```svelte
<VitalSignWeightAsKgInput label="Weight (kg)" bind:value />
```
