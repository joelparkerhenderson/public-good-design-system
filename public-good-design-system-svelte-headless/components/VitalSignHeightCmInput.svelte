<script lang="ts">
    // VitalSignHeightCmInput component
    //
    ...
    //   <VitalSignHeightCmInput label="Height (cm)" bind:value min={50} max={250} required />
    //
    // Keyboard:
...
<!-- VitalSignHeightCmInput.svelte -->
<input
    class={`vital-sign-height-cm-input ${className}`}
    type="number"
    aria-label={label}
    bind:value
    {min}
