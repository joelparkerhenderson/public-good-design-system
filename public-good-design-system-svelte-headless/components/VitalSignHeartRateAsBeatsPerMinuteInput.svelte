<script lang="ts">
    ...
    // sensible defaults for heart rate entry (min 0, max 300, step 1). It provides
    ...
    //
    // Syntax:
    //   <VitalSignHeartRateAsBeatsPerMinuteInput label="Heart rate (bpm)" bind:value />
    //
    // Examples:
    //   <!-- Basic heart rate input -->
    //   <VitalSignHeartRateAsBeatsPerMinuteInput label="Heart rate (bpm)" bind:value />
    //
    //   <!-- Required heart rate input with custom range -->
    //   <VitalSignHeartRateAsBeatsPerMinuteInput label="Heart rate (bpm)" bind:value min={30} max={250} required />
    //
    // Keyboard:
...
<!-- VitalSignHeartRateAsBeatsPerMinuteInput.svelte -->
<input
    class={`vital-sign-heart-rate-as-beats-per-minute-input ${className}`}
    type="number"
    aria-label={label}
    bind:value
    {min}
