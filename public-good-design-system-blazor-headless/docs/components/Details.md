...
- Uses `@bind-` on the `open` prop with Blazor's `@bind-Open` directive for two-way state synchronization
- The `summary` prop provides the clickable toggle text
- Accepts a `ChildContent` RenderFragment for the expandable content area
