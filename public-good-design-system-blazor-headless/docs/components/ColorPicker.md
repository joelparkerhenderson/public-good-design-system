...
- Uses `@bind-` for both `x` and `y` coordinates (range 0-100)
...
| `X` | `int` | `0` | The x value |
| `XChanged` | `EventCallback<int>` | `—` | Event callback for two-way binding of X |
| `Y` | `int` | `0` | The y value |
| `YChanged` | `EventCallback<int>` | `—` | Event callback for two-way binding of Y |
