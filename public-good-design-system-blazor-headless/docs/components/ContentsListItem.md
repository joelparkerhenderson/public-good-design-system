...
(usually an anchor link with descriptive text) through the ChildContent render fragment, and
...
- All content is provided through the ChildContent render fragment (typically links)
...
- Spreads `AdditionalAttributes` onto the `<li>` element for consumer customization
