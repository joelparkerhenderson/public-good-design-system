...
- Uses Blazor 5 `private field` rune to track visibility; dismissing sets `visible` to `false`
...
- Uses `ChildContent` RenderFragment for flexible banner content rendering
- Spreads `AdditionalAttributes` on the root `<div>` element for consumer extensibility
...
| `Dismissible` | `bool` | `—` | The dismissible value |
| `CloseLabel` | `string?` | `—` | The close label value |
