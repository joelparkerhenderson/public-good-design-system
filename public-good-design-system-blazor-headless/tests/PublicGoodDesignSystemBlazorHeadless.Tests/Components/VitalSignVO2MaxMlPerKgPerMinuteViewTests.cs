...
public class VitalSignVO2MaxMlPerKgPerMinuteViewTests : TestContext
{
    [Fact]
    public void RendersAsSpan()
    {
        var cut = RenderComponent<VitalSignVO2MaxMlPerKgPerMinuteView>();
    ...
    public void HasBaseClass()
    {
        var cut = RenderComponent<VitalSignVO2MaxMlPerKgPerMinuteView>();
        var element = cut.Find("span");
        Assert.Contains("vital-sign-vo2-max-ml-per-kg-per-minute-view", element.GetAttribute("class"));
    }

    [Fact]
    public void MergesCssClass()
    {
        var cut = RenderComponent<VitalSignVO2MaxMlPerKgPerMinuteView>(p => p
            .Add(c => c.CssClass, "custom-class"));
        var element = cut.Find("span");
        var classes = element.GetAttribute("class");
        Assert.Contains("vital-sign-vo2-max-ml-per-kg-per-minute-view", classes);
