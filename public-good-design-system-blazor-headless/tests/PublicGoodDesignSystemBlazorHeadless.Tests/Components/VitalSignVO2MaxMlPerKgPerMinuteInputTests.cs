...
public class VitalSignVO2MaxMlPerKgPerMinuteInputTests : TestContext
{
    [Fact]
    public void RendersAsInput()
    {
        var cut = RenderComponent<VitalSignVO2MaxMlPerKgPerMinuteInput>();
    ...
    public void HasBaseClass()
    {
        var cut = RenderComponent<VitalSignVO2MaxMlPerKgPerMinuteInput>();
        var element = cut.Find("input");
        Assert.Contains("vital-sign-vo2-max-ml-per-kg-per-minute-input", element.GetAttribute("class"));
    }

    [Fact]
    public void MergesCssClass()
    {
        var cut = RenderComponent<VitalSignVO2MaxMlPerKgPerMinuteInput>(p => p
            .Add(c => c.CssClass, "custom-class"));
        var element = cut.Find("input");
        var classes = element.GetAttribute("class");
        Assert.Contains("vital-sign-vo2-max-ml-per-kg-per-minute-input", classes);
