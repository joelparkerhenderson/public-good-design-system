...
public class VitalSignVo2MaxAsMlPerKgPerMinuteInputTests : TestContext
{
    [Fact]
    public void RendersAsInput()
    {
        var cut = RenderComponent<VitalSignVo2MaxAsMlPerKgPerMinuteInput>();
    ...
    public void HasBaseClass()
    {
        var cut = RenderComponent<VitalSignVo2MaxAsMlPerKgPerMinuteInput>();
        var element = cut.Find("input");
        Assert.Contains("vital-sign-vo2-max-as-ml-per-kg-per-minute-input", element.GetAttribute("class"));
    }

    [Fact]
    public void MergesCssClass()
    {
        var cut = RenderComponent<VitalSignVo2MaxAsMlPerKgPerMinuteInput>(p => p
            .Add(c => c.CssClass, "custom-class"));
        var element = cut.Find("input");
        var classes = element.GetAttribute("class");
        Assert.Contains("vital-sign-vo2-max-as-ml-per-kg-per-minute-input", classes);
    ...
    {
        var cut = RenderComponent<VitalSignVo2MaxAsMlPerKgPerMinuteInput>(p => p
            .Add(c => c.Label, "VO2 max (mL/kg/min)"));
        var element = cut.Find("input");
        Assert.Equal("VO2 max (mL/kg/min)", element.GetAttribute("aria-label"));
    }
