...
public class VitalSignCholesterolAsHdlMmolPerLitreInputTests : TestContext
{
    [Fact]
    public void RendersAsInput()
    {
        var cut = RenderComponent<VitalSignCholesterolAsHdlMmolPerLitreInput>();
    ...
    public void HasBaseClass()
    {
        var cut = RenderComponent<VitalSignCholesterolAsHdlMmolPerLitreInput>();
        var element = cut.Find("input");
        Assert.Contains("vital-sign-cholesterol-as-hdl-mmol-per-litre-input", element.GetAttribute("class"));
    }

    [Fact]
    public void MergesCssClass()
    {
        var cut = RenderComponent<VitalSignCholesterolAsHdlMmolPerLitreInput>(p => p
            .Add(c => c.CssClass, "custom-class"));
        var element = cut.Find("input");
        var classes = element.GetAttribute("class");
        Assert.Contains("vital-sign-cholesterol-as-hdl-mmol-per-litre-input", classes);
    ...
    {
        var cut = RenderComponent<VitalSignCholesterolAsHdlMmolPerLitreInput>(p => p
            .Add(c => c.Label, "Cholesterol HDL (mmol/L)"));
        var element = cut.Find("input");
        Assert.Equal("Cholesterol HDL (mmol/L)", element.GetAttribute("aria-label"));
    }
