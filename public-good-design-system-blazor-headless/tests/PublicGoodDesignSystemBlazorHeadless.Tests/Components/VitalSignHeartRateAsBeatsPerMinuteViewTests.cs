...
public class VitalSignHeartRateAsBeatsPerMinuteViewTests : TestContext
{
    [Fact]
    public void RendersAsSpan()
    {
        var cut = RenderComponent<VitalSignHeartRateAsBeatsPerMinuteView>();
    ...
    public void HasBaseClass()
    {
        var cut = RenderComponent<VitalSignHeartRateAsBeatsPerMinuteView>();
        var element = cut.Find("span");
        Assert.Contains("vital-sign-heart-rate-as-beats-per-minute-view", element.GetAttribute("class"));
    }

    [Fact]
    public void MergesCssClass()
    {
        var cut = RenderComponent<VitalSignHeartRateAsBeatsPerMinuteView>(p => p
            .Add(c => c.CssClass, "custom-class"));
        var element = cut.Find("span");
        var classes = element.GetAttribute("class");
        Assert.Contains("vital-sign-heart-rate-as-beats-per-minute-view", classes);
