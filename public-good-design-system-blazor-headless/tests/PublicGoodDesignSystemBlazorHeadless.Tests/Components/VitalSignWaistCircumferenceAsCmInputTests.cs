...
public class VitalSignWaistCircumferenceAsCmInputTests : TestContext
{
    [Fact]
    public void RendersAsInput()
    {
        var cut = RenderComponent<VitalSignWaistCircumferenceAsCmInput>();
    ...
    public void HasBaseClass()
    {
        var cut = RenderComponent<VitalSignWaistCircumferenceAsCmInput>();
        var element = cut.Find("input");
        Assert.Contains("vital-sign-waist-circumference-as-cm-input", element.GetAttribute("class"));
    }

    [Fact]
    public void MergesCssClass()
    {
        var cut = RenderComponent<VitalSignWaistCircumferenceAsCmInput>(p => p
            .Add(c => c.CssClass, "custom-class"));
        var element = cut.Find("input");
        var classes = element.GetAttribute("class");
        Assert.Contains("vital-sign-waist-circumference-as-cm-input", classes);
    ...
    {
        var cut = RenderComponent<VitalSignWaistCircumferenceAsCmInput>(p => p
            .Add(c => c.Label, "Waist circumference (cm)"));
        var element = cut.Find("input");
        Assert.Equal("Waist circumference (cm)", element.GetAttribute("aria-label"));
    }
