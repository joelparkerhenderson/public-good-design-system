...
public class VitalSignHeartRateAsBeatsPerMinuteInputTests : TestContext
{
    [Fact]
    public void RendersAsInput()
    {
        var cut = RenderComponent<VitalSignHeartRateAsBeatsPerMinuteInput>();
    ...
    public void HasBaseClass()
    {
        var cut = RenderComponent<VitalSignHeartRateAsBeatsPerMinuteInput>();
        var element = cut.Find("input");
        Assert.Contains("vital-sign-heart-rate-as-beats-per-minute-input", element.GetAttribute("class"));
    }

    [Fact]
    public void MergesCssClass()
    {
        var cut = RenderComponent<VitalSignHeartRateAsBeatsPerMinuteInput>(p => p
            .Add(c => c.CssClass, "custom-class"));
        var element = cut.Find("input");
        var classes = element.GetAttribute("class");
        Assert.Contains("vital-sign-heart-rate-as-beats-per-minute-input", classes);
