...
public class VitalSignRespiratoryRateAsBreathsPerMinuteInputTests : TestContext
{
    [Fact]
    public void RendersAsInput()
    {
        var cut = RenderComponent<VitalSignRespiratoryRateAsBreathsPerMinuteInput>();
    ...
    public void HasBaseClass()
    {
        var cut = RenderComponent<VitalSignRespiratoryRateAsBreathsPerMinuteInput>();
        var element = cut.Find("input");
        Assert.Contains("vital-sign-respiratory-rate-as-breaths-per-minute-input", element.GetAttribute("class"));
    }

    [Fact]
    public void MergesCssClass()
    {
        var cut = RenderComponent<VitalSignRespiratoryRateAsBreathsPerMinuteInput>(p => p
            .Add(c => c.CssClass, "custom-class"));
        var element = cut.Find("input");
        var classes = element.GetAttribute("class");
        Assert.Contains("vital-sign-respiratory-rate-as-breaths-per-minute-input", classes);
    ...
    {
        var cut = RenderComponent<VitalSignRespiratoryRateAsBreathsPerMinuteInput>(p => p
            .Add(c => c.Label, "Respiratory rate (breaths/min)"));
        var element = cut.Find("input");
        Assert.Equal("Respiratory rate (breaths/min)", element.GetAttribute("aria-label"));
    }
