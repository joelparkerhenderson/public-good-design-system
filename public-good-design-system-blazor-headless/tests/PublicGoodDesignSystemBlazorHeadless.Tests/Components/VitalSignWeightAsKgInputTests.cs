...
public class VitalSignWeightAsKgInputTests : TestContext
{
    [Fact]
    public void RendersAsInput()
    {
        var cut = RenderComponent<VitalSignWeightAsKgInput>();
    ...
    public void HasBaseClass()
    {
        var cut = RenderComponent<VitalSignWeightAsKgInput>();
        var element = cut.Find("input");
        Assert.Contains("vital-sign-weight-as-kg-input", element.GetAttribute("class"));
    }

    [Fact]
    public void MergesCssClass()
    {
        var cut = RenderComponent<VitalSignWeightAsKgInput>(p => p
            .Add(c => c.CssClass, "custom-class"));
        var element = cut.Find("input");
        var classes = element.GetAttribute("class");
        Assert.Contains("vital-sign-weight-as-kg-input", classes);
    ...
    {
        var cut = RenderComponent<VitalSignWeightAsKgInput>(p => p
            .Add(c => c.Label, "Weight (kg)"));
        var element = cut.Find("input");
        Assert.Equal("Weight (kg)", element.GetAttribute("aria-label"));
    }
