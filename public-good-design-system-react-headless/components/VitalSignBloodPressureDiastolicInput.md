...
VitalSignBloodPressureDiastolicInput is a headless component that wraps a native `<input type="number">` element for entering a diastolic blood pressure value in millimeters of mercury (mmHg). It provides accessible labelling and physiologically appropriate default constraints (0-200 mmHg).
...
- Default `min=0`, `max=200`, `step=1` for typical diastolic blood pressure ranges
...
- `value`: number | undefined (default: `undefined`) -- current diastolic blood pressure value; controlled with `value + onChange`
