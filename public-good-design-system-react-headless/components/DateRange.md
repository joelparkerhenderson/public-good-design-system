...
- Both `start` and `end` support controlled component usage
...
- `start`: string (default: "") -- start date value in YYYY-MM-DD format; controlled
- `end`: string (default: "") -- end date value in YYYY-MM-DD format; controlled
...
<DateRange
    ...
    endLabel="Return"
    start={startDate} onStartChange={setStartDate}
    end={endDate} onEndChange={setEndDate}
/>
```
