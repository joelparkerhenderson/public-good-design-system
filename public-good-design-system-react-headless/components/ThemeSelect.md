...
The consumer provides ThemeSelectOption elements as children to define the available themes. The component uses `aria-label` for accessible naming and supports controlled component usage for the selected theme.
...
- Uses `ReactNode` for the `children` prop to render ThemeSelectOption elements
...
- `value`: string (default: "") -- controlled string representing the currently selected theme
...
```tsx
<ThemeSelect label="Theme" value + onChange>
