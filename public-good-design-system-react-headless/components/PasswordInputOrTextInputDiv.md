...
- Uses `useState(false)` to track password visibility internally
...
- `value`: string (default: `""`) -- current password value; controlled with `value + onChange`
...
```tsx
<PasswordInputOrTextInputDiv label="Password" value + onChange />
```
