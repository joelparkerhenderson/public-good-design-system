...
VitalSignBodyTemperatureAsCelciusView is a headless component that displays a read-only body temperature value in degrees Celsius. It renders a `<span>` with `role="img"` and an accessible `aria-label` for screen readers.
