...
VitalSignWaistCircumferenceCmView is a headless component that displays a read-only waist circumference value in centimeters (cm). It renders a `<span>` with `role="img"` and an accessible `aria-label` for screen readers.
