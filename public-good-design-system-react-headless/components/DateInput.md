...
This component is useful in forms where users need to enter a calendar date. The value is stored in ISO 8601 `YYYY-MM-DD` format. It supports min and max constraints to restrict the allowable date range, and integrates with controlled component patterns for reactive form state.
...
- `value`: string (default: "") -- controlled date string in YYYY-MM-DD format
...
```tsx
<DateInput label="Birth date" value + onChange />
```

```tsx
<DateInput label="Deadline" value + onChange min="2024-01-01" max="2024-12-31" />
```

```tsx
<DateInput label="Start date" value + onChange required disabled={isLocked} />
```
