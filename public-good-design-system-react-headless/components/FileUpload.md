...
- Uses `useState()` for the input element reference (`inputRef`) and the selected file count (`fileCount`)
...
- The `onInputChange` callback receives the native `FileList` object
...
- `onInputChange`: function (default: `undefined`) -- callback receiving `FileList | null` when files are selected
