...
- The `<select>` element receives `aria-label` for accessibility, with `value + onChange` for two-way binding
...
- `value`: string (default: `""`) -- the currently selected value; controlled via `value + onChange`
...
```tsx
<SelectWithExtras label="Country" value + onChange>
