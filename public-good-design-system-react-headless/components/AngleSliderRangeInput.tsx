...
//     valueTextSuffix=" degrees" value={hue} onChange={setHue} />
//
//   
...
//   - The value prop must use controlled for two-way binding
...
export interface AngleSliderRangeInputProps {
    className?: string;
...
export default function AngleSliderRangeInput({
    className = "",
    value = 0,
    min = 0,
    max = 360,
    ...
    onChange,
    ...restProps
}: AngleSliderRangeInputProps) {
    const valueText = `${value}${valueTextSuffix}`;

    return (
        <input
        className={`angle-slider-range-input ${className}`}
        type="range"
