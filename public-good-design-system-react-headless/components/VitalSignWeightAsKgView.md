...
VitalSignWeightAsKgView is a headless component that displays a read-only weight value in kilograms (kg). It renders a `<span>` with `role="img"` and an accessible `aria-label` for screen readers.
