...
NumberInput is a headless component that wraps a native `<input type="number">` element with accessible labelling and optional min, max, and step constraints. It provides a controlled numeric value for two-way data flow.
...
- `value`: number | undefined (default: `undefined`) -- current numeric value; controlled with `value + onChange`
...
```tsx
<NumberInput label="Quantity" value + onChange min={0} max={100} step={1} />
```
