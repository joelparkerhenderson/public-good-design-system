...
VitalSignBodyTemperatureAsCelciusInput is a headless component that wraps a native `<input type="number">` element for entering a body temperature value in degrees Celsius. It provides accessible labelling and physiologically appropriate default constraints (30-45 degrees Celsius).
...
- Default `min=30`, `max=45`, `step=0.1` for typical body temperature ranges
...
- `value`: number | undefined (default: `undefined`) -- current body temperature value; controlled with `value + onChange`
