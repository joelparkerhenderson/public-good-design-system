...
VitalSignBloodPressureSystolicView is a headless component that displays a read-only systolic blood pressure value in millimeters of mercury (mmHg). It renders a `<span>` with `role="img"` and an accessible `aria-label` for screen readers.
