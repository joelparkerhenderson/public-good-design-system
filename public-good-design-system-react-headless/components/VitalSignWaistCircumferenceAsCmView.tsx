...
// to VitalSignWaistCircumferenceAsCmInput, used for showing waist circumference
...
export interface VitalSignWaistCircumferenceAsCmViewProps {
    className?: string;
...
export default function VitalSignWaistCircumferenceAsCmView({
    className = "",
    value,
    label,
    ...restProps
}: VitalSignWaistCircumferenceAsCmViewProps) {
    return (
        <span
            className={`vital-sign-waist-circumference-as-cm-view ${className}`}
            role="img"
            aria-label={label}
            data-value={value}
            {...restProps}
