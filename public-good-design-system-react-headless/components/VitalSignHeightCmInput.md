...
VitalSignHeightCmInput is a headless component that wraps a native `<input type="number">` element for entering a height value in centimeters (cm). It provides accessible labelling and physiologically appropriate default constraints (0-300 cm).
...
- Default `min=0`, `max=300`, `step=1` for typical height ranges
...
- `value`: number | undefined (default: `undefined`) -- current height value; controlled with `value + onChange`
...
```tsx
<VitalSignHeightCmInput label="Height (cm)" value={value} onChange={setValue} />
```
