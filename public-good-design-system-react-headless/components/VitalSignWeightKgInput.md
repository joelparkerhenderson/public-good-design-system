...
VitalSignWeightKgInput is a headless component that wraps a native `<input type="number">` element for entering a weight value in kilograms (kg). It provides accessible labelling and physiologically appropriate default constraints (0-500 kg).
...
- Default `min=0`, `max=500`, `step=1` for typical weight ranges
...
- `value`: number | undefined (default: `undefined`) -- current weight value; controlled with `value + onChange`
