...
- Uses `useState` to track visibility; dismissing sets `visible` to `false`
...
- Uses `children` ReactNode for flexible banner content rendering
- Spreads `...restProps` on the root `<div>` element for consumer extensibility
- Entire banner is conditionally rendered when `visible` is true
