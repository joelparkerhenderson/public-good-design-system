...
VitalSignWaistCircumferenceCmInput is a headless component that wraps a native `<input type="number">` element for entering a waist circumference value in centimeters (cm). It provides accessible labelling and physiologically appropriate default constraints (0-300 cm).
...
- Default `min=0`, `max=300`, `step=1` for typical waist circumference ranges
...
- `value`: number | undefined (default: `undefined`) -- current waist circumference value; controlled with `value + onChange`
...
```tsx
<VitalSignWaistCircumferenceCmInput label="Waist circumference (cm)" value={value} onChange={setValue} />
```
