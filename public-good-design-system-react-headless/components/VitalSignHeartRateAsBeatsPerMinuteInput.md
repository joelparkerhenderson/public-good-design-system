...
VitalSignHeartRateAsBeatsPerMinuteInput is a headless component that wraps a native `<input type="number">` element for entering a heart rate value in beats per minute (BPM). It provides accessible labelling and physiologically appropriate default constraints (0-300 BPM).
...
- Default `min=0`, `max=300`, `step=1` for typical heart rate ranges
...
- `value`: number | undefined (default: `undefined`) -- current heart rate value; controlled with `value + onChange`
