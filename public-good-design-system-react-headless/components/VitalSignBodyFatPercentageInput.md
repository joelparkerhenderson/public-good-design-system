...
VitalSignBodyFatPercentageInput is a headless component that wraps a native `<input type="number">` element for entering a body fat percentage value. It provides accessible labelling and appropriate default constraints (0-100 percent).
...
- Default `min=0`, `max=100`, `step=0.1` for typical body fat percentage ranges
...
- `value`: number | undefined (default: `undefined`) -- current body fat percentage value; controlled with `value + onChange`
...
```tsx
<VitalSignBodyFatPercentageInput label="Body fat percentage" value={value} onChange={setValue} />
```
