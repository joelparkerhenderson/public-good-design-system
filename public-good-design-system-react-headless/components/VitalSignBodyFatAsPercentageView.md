...
VitalSignBodyFatAsPercentageView is a headless component that displays a read-only body fat percentage value. It renders a `<span>` with `role="img"` and an accessible `aria-label` for screen readers.
