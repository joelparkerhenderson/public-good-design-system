...
- `value`: string (default: "") -- current date value in YYYY-MM-DD format, controlled via `value + onChange`
...
```tsx
<DateField label="Start date" value + onChange />
```

```tsx
<DateField label="End date" value + onChange error="Required" required />
```

```tsx
<DateField label="Birthday" value + onChange description="Format: YYYY-MM-DD" />
```
