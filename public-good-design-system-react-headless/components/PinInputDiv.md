...
available as a single controlled string. This pattern is commonly used in
...
- Uses `useState()` for internal digits array and supports controlled component usage for combined value
...
- `value`: string (default: "") -- controlled combined string of all digits
...
```tsx
<PinInputDiv label="OTP" value={otp} onChange={setOtp} data-form="login" />
```
