...
export interface FiveFaceRatingViewProps {
    className?: string;
    /** Rating value (1-5). */
    value: number;
...
export default function FiveFaceRatingView({
    className = "",
    value,
    label,
    labels = ["Very bad", "Bad", "Okay", "Good", "Very good"],
    ...restProps
}: FiveFaceRatingViewProps) {
    const faceLabel = labels[value - 1] ?? "";

    return (
        <span
        className={`five-face-rating-view ${className}`}
        role="img"
        aria-label={label}
        data-value={value}
        {...restProps}
