...
VitalSignCholesterolLdlMmolPerLitreInput is a headless component that wraps a native `<input type="number">` element for entering an LDL cholesterol value in millimoles per litre. It provides accessible labelling and clinically appropriate default constraints (0-10 mmol/L).
...
- Default `min=0`, `max=10`, `step=0.1` for typical LDL cholesterol ranges
...
- `value`: number | undefined (default: `undefined`) -- current LDL cholesterol value; controlled with `value + onChange`
