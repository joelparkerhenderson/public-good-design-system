...
VitalSignHeartRateBeatsPerMinuteInput is a headless component that wraps a native `<input type="number">` element for entering a heart rate value in beats per minute (BPM). It provides accessible labelling and physiologically appropriate default constraints (0-300 BPM).
