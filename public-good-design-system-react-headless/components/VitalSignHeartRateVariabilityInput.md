...
VitalSignHeartRateVariabilityInput is a headless component that wraps a native `<input type="number">` element for entering a heart rate variability (HRV) value in milliseconds. It provides accessible labelling and physiologically appropriate default constraints (0-500 ms).
...
- Default `min=0`, `max=500`, `step=1` for typical HRV ranges
...
- `value`: number | undefined (default: `undefined`) -- current HRV value; controlled with `value + onChange`
