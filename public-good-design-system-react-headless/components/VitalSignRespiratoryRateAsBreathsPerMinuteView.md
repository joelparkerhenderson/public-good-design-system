...
VitalSignRespiratoryRateAsBreathsPerMinuteView is a headless component that displays a read-only respiratory rate value in breaths per minute. It renders a `<span>` with `role="img"` and an accessible `aria-label` for screen readers.
