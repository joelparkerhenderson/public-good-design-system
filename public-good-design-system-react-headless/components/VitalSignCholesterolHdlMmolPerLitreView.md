...
VitalSignCholesterolHdlMmolPerLitreView is a headless component that displays a read-only HDL cholesterol value in millimoles per litre. It renders a `<span>` with `role="img"` and an accessible `aria-label` for screen readers.
