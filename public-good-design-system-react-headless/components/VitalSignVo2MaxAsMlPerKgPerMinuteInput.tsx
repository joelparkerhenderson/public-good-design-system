// VitalSignVO2MaxMlPerKgPerMinuteInput component
//
// A headless numeric input for entering a VO2 max value in milliliters per
// kilogram per minute (ml/kg/min). Wraps a native <input type="number"> element
...
// Commonly used in fitness assessment forms, health monitoring applications,
// and athletic performance record entry.
//
// Props:
//   className — string, optional. CSS class name.
//   label — string, required. Accessible name for the input via aria-label.
//   value — number | undefined, default undefined. Current VO2 max value; controlled.
...
//
//
//   <VitalSignVO2MaxMlPerKgPerMinuteInput label="VO2 max" value={value} onChange={setValue} required />
//
// Keyboard:
...
export interface VitalSignVO2MaxMlPerKgPerMinuteInputProps {
    className?: string;
    label: string;
...
export default function VitalSignVO2MaxMlPerKgPerMinuteInput({
    className = "",
    label,
    value = undefined,
    min = 0,
    ...
    onChange,
    ...restProps
}: VitalSignVO2MaxMlPerKgPerMinuteInputProps) {
    return (
        <input
            className={`vital-sign-vo2-max-as-ml-per-kg-per-minute-input ${className}`}
            type="number"
            aria-label={label}
            value={value}
