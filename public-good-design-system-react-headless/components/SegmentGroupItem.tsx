...
//   children — ReactNode, required. Label content to render inside the button.
...
//     <SegmentGroupItem checked={mode === 'grid'} value="grid" onClick={() => setMode('grid')}>Grid</SegmentGroupItem>
//     <SegmentGroupItem checked={mode === 'list'} value="list" onClick={() => setMode('list')}>List</SegmentGroupItem>
//   </SegmentGroup>
//
// Keyboard:
...
//   - Label content comes through the children prop for localization
//
// Claude rules:
...
export default function SegmentGroupItem({
    className = "",
    checked = false,
    value = "",
    disabled = false,
    ...
    return (
        <button
        className={`segment-group-item ${className}`}
        role="radio"
