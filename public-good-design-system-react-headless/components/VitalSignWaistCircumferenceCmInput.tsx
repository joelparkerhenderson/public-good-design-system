...
//
//
//   <VitalSignWaistCircumferenceCmInput label="Waist circumference (cm)" value={value} onChange={setValue} required />
//
// Keyboard:
...
export interface VitalSignWaistCircumferenceCmInputProps {
    className?: string;
    label: string;
...
export default function VitalSignWaistCircumferenceCmInput({
    className = "",
    label,
    value = undefined,
    min = 0,
    ...
    onChange,
    ...restProps
}: VitalSignWaistCircumferenceCmInputProps) {
    return (
        <input
            className={`vital-sign-waist-circumference-cm-input ${className}`}
            type="number"
            aria-label={label}
            value={value}
