...
The component supports controlled value state, required and disabled attributes, and spreads additional props for full consumer control over the native `<input>` element.
...
- `value`: string (default: `""`) -- controlled current input value.
...
```tsx
<Input label="Name" value + onChange type="text" required />
```

```tsx
<Input label="Email address" value + onChange type="email" required disabled={isSubmitting} />
```
