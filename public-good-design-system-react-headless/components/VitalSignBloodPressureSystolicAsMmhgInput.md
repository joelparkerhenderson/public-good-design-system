...
VitalSignBloodPressureSystolicAsMmhgInput is a headless component that wraps a native `<input type="number">` element for entering a systolic blood pressure value in millimeters of mercury (mmHg). It provides accessible labelling and physiologically appropriate default constraints (0-300 mmHg).
...
- Default `min=0`, `max=300`, `step=1` for typical systolic blood pressure ranges
...
- `value`: number | undefined (default: `undefined`) -- current systolic blood pressure value; controlled with `value + onChange`
