...
//   children — ReactNode, required. Event description content.
...
export default function TimelineListItem({
    ...
    return (
        <li
        className={`timeline-list-item ${className}`}
        {...restProps}
        >
        {(datetime || heading) && (<time dateTime={datetime || undefined}>{heading}</time>)}
        {children}
        </li>
    );
}
