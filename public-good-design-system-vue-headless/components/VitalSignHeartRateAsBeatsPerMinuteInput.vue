<script setup lang="ts">
    ...
    // and sensible default constraints (0-300 BPM). Commonly used in medical forms,
    // patient intake screens, and health monitoring applications.
    //
    // Props:
    //   label — string, required. Accessible name for the input via aria-label.
    //   value — number | undefined, default undefined. Current heart rate value; bindable.
    ...
    //
    // Syntax:
    //   <VitalSignHeartRateAsBeatsPerMinuteInput label="Heart rate" v-model />
    //
    // Examples:
    //   <!-- Heart rate input with defaults -->
    //   <VitalSignHeartRateAsBeatsPerMinuteInput label="Heart rate (BPM)" v-model />
    //
    //   <!-- Required heart rate input -->
    //   <VitalSignHeartRateAsBeatsPerMinuteInput label="Heart rate" v-model required />
    //
    // Keyboard:
...
<template>
    <!-- VitalSignHeartRateAsBeatsPerMinuteInput.vue -->
    <input
        class="vital-sign-heart-rate-as-beats-per-minute-input"
        type="number"
        :aria-label="label"
        v-model="modelValue"
        :min="min"
