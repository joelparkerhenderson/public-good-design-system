...
VitalSignRespiratoryRateAsBreathsPerMinuteInput is a headless component that wraps a native `<input type="number">` element for entering a respiratory rate value in breaths per minute. It provides accessible labelling and sensible default constraints (0-100 breaths per minute) with a bindable numeric value for two-way data flow.

This component is useful for medical forms, patient intake screens, health monitoring applications, and any context where respiratory rate data needs to be entered.
...
- `value`: number | undefined (default: `undefined`) -- current respiratory rate value; bindable with `v-model`
...
```vue
<VitalSignRespiratoryRateAsBreathsPerMinuteInput label="Respiratory rate (breaths/min)" v-model="respiratoryRate" />
```
