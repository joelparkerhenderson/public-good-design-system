...
VitalSignWaistCircumferenceCmView is a headless component that displays a read-only waist circumference value in centimeters (cm). It renders a `<span>` element with `role="img"` and an accessible label for screen readers.
