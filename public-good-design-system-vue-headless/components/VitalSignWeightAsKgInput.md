...
VitalSignWeightAsKgInput is a headless component that wraps a native `<input type="number">` element for entering a weight value in kilograms (kg). It provides accessible labelling and sensible default constraints (0-500 kg) with a bindable numeric value for two-way data flow.
...
```vue
<VitalSignWeightAsKgInput label="Weight (kg)" v-model="weight" />
```
