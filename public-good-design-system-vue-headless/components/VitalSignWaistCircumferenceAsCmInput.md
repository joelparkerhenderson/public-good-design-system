...
VitalSignWaistCircumferenceAsCmInput is a headless component that wraps a native `<input type="number">` element for entering a waist circumference value in centimeters (cm). It provides accessible labelling and sensible default constraints (0-300 cm) with a bindable numeric value for two-way data flow.
...
```vue
<VitalSignWaistCircumferenceAsCmInput label="Waist circumference (cm)" v-model="waistCircumference" />
```
