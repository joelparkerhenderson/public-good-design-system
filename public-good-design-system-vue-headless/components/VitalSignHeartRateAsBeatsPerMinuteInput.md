...
VitalSignHeartRateAsBeatsPerMinuteInput is a headless component that wraps a native `<input type="number">` element for entering a heart rate value in beats per minute (BPM). It provides accessible labelling and sensible default constraints (0-300 BPM) with a bindable numeric value for two-way data flow.

This component is useful for medical forms, patient intake screens, health monitoring applications, and any context where heart rate data needs to be entered.
...
- `value`: number | undefined (default: `undefined`) -- current heart rate value; bindable with `v-model`
...
```vue
<VitalSignHeartRateAsBeatsPerMinuteInput label="Heart rate (BPM)" v-model="heartRate" />
```
