...
VitalSignHeartRateVariabilityInput is a headless component that wraps a native `<input type="number">` element for entering a heart rate variability (HRV) value in milliseconds. It provides accessible labelling and sensible default constraints (0-500 ms) with a bindable numeric value for two-way data flow.

This component is useful for medical forms, patient intake screens, health monitoring applications, and any context where heart rate variability data needs to be entered.
...
- Default constraints: `min=0`, `max=500`, `step=1`
...
- `value`: number | undefined (default: `undefined`) -- current HRV value; bindable with `v-model`
...
```vue
<VitalSignHeartRateVariabilityInput label="HRV (ms)" v-model="hrv" />
```
