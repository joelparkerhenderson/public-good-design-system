...
VitalSignBodyFatAsPercentageInput is a headless component that wraps a native `<input type="number">` element for entering a body fat percentage value. It provides accessible labelling and sensible default constraints (0-100 %) with a bindable numeric value for two-way data flow.
...
```vue
<VitalSignBodyFatAsPercentageInput label="Body fat percentage (%)" v-model="bodyFatPercentage" />
```
