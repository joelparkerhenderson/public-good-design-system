...
VitalSignWeightKgView is a headless component that displays a read-only weight value in kilograms (kg). It renders a `<span>` element with `role="img"` and an accessible label for screen readers.
