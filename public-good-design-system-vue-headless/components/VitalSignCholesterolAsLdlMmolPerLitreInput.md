...
VitalSignCholesterolAsLdlMmolPerLitreInput is a headless component that wraps a native `<input type="number">` element for entering an LDL cholesterol value in millimoles per litre. It provides accessible labelling and sensible default constraints (0-10 mmol/L) with a bindable numeric value for two-way data flow.

This component is useful for medical forms, patient intake screens, health monitoring applications, and any context where LDL cholesterol data needs to be entered.
...
- Default constraints: `min=0`, `max=10`, `step=0.1`
...
- `value`: number | undefined (default: `undefined`) -- current LDL cholesterol value; bindable with `v-model`
...
```vue
<VitalSignCholesterolAsLdlMmolPerLitreInput label="LDL cholesterol (mmol/L)" v-model="ldlCholesterol" />
```
