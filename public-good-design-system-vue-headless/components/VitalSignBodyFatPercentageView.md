...
VitalSignBodyFatPercentageView is a headless component that displays a read-only body fat percentage value. It renders a `<span>` element with `role="img"` and an accessible label for screen readers.
