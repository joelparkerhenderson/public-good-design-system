...
VitalSignWeightKgInput is a headless component that wraps a native `<input type="number">` element for entering a weight value in kilograms (kg). It provides accessible labelling and sensible default constraints (0-500 kg) with a bindable numeric value for two-way data flow.

This component is useful for medical forms, patient intake screens, health monitoring applications, and any context where weight data needs to be entered.
...
- `value`: number | undefined (default: `undefined`) -- current weight value; bindable with `v-model`
...
```vue
<VitalSignWeightKgInput label="Weight (kg)" v-model="weight" />
```
