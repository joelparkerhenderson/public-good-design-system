...
VitalSignHeightCmInput is a headless component that wraps a native `<input type="number">` element for entering a height value in centimeters (cm). It provides accessible labelling and sensible default constraints (0-300 cm) with a bindable numeric value for two-way data flow.

This component is useful for medical forms, patient intake screens, health monitoring applications, and any context where height data needs to be entered.
...
- `value`: number | undefined (default: `undefined`) -- current height value; bindable with `v-model`
...
```vue
<VitalSignHeightCmInput label="Height (cm)" v-model="height" />
```
