...
VitalSignBloodPressureDiastolicInput is a headless component that wraps a native `<input type="number">` element for entering a diastolic blood pressure value in millimeters of mercury (mmHg). It provides accessible labelling and sensible default constraints (0-200 mmHg) with a bindable numeric value for two-way data flow.

This component is useful for medical forms, patient intake screens, health monitoring applications, and any context where diastolic blood pressure data needs to be entered.
...
- Default constraints: `min=0`, `max=200`, `step=1`
...
- `value`: number | undefined (default: `undefined`) -- current diastolic blood pressure value; bindable with `v-model`
...
```vue
<VitalSignBloodPressureDiastolicInput label="Diastolic blood pressure (mmHg)" v-model="diastolic" />
```
