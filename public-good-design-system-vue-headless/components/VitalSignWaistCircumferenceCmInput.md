# VitalSignWaistCircumferenceCmInput

VitalSignWaistCircumferenceCmInput is a headless component that wraps a native `<input type="number">` element for entering a waist circumference value in centimeters (cm). It provides accessible labelling and sensible default constraints (0-300 cm) with a bindable numeric value for two-way data flow.

This component is useful for medical forms, patient intake screens, health monitoring applications, and any context where waist circumference data needs to be entered.
...
- Default constraints: `min=0`, `max=300`, `step=1`
...
- `value`: number | undefined (default: `undefined`) -- current waist circumference value; bindable with `v-model`
...
```vue
<VitalSignWaistCircumferenceCmInput label="Waist circumference (cm)" v-model="waistCircumference" />
```
