...
VitalSignHeartRateBeatsPerMinuteInput is a headless component that wraps a native `<input type="number">` element for entering a heart rate value in beats per minute (BPM). It provides accessible labelling and sensible default constraints (0-300 BPM) with a bindable numeric value for two-way data flow.
...
```vue
<VitalSignHeartRateBeatsPerMinuteInput label="Heart rate (BPM)" v-model="heartRate" />
```
