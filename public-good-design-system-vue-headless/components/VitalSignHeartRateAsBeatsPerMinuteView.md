...
VitalSignHeartRateAsBeatsPerMinuteView is a headless component that displays a read-only heart rate value in beats per minute (BPM). It renders a `<span>` element with `role="img"` and an accessible label for screen readers.
